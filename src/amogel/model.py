...
""" Class for DNN for other model evaluation"""
class DNN(pl.LightningModule):
    ...
    def __init__(self , input_dimension,  num_classes) -> None:
        ...
        self.acc_train = Accuracy(num_classes=num_classes , task="multiclass")
        self.acc_val = Accuracy(num_classes=num_classes , task="multiclass")
        self.actual = []
        self.predicted = []
        self.predicted_proba = []
    ...
    def training_step(self, batch, batch_idx):
        ...
        acc = self.acc_train(F.softmax(y_hat , dim=1) , y)
    ...
    def validation_step(self, batch , batch_idx):
        ...
        # calculate loss
        loss = self.loss(y_hat , y)
        acc = self.acc_val(F.softmax(y_hat , dim=1) , y)
        
        self.actual.extend(y.cpu().numpy())
        self.predicted.extend(F.softmax(y_hat , dim=1).argmax(dim=1).cpu().numpy())
        self.predicted_proba.extend(F.softmax(y_hat , dim=-1).cpu().numpy())  
...
if __name__ == "__main__": 
    ...
    model = GCN(
        ...
        DNN=params['model']['dnn'], 
        GNN=params['model']['gnn'] 
    )
    ...
    with mlflow.start_run() as run:
        ...
        mlflow.log_param('seed' , params['data_preprocessing']['seed'])
